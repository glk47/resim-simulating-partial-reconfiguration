...
////lingkang: change memory model name from ddr2_model to ddr2
module ddr2 (
    ck,
    ck_n,
    cke,
    ...
    ////lingkang: the parameter file
    `include "ddr2.v5_82.param.vh"
...
/*
    lingkang: 
        by default: 
            `define MAX_SIZE (1<<(BA_BITS+ROW_BITS+COL_BITS-BL_BITS)) // for Linux
            `define MEM_SIZE (1<<MEM_BITS) // for WindowsXP
        now i change it to: 
            `define MAX_SIZE (1<<MEM_BITS) // for both Linux and WindowsXP
            `define MEM_SIZE (1<<MEM_BITS) // not used any more
*/
    `define MAX_SIZE   (1<<MEM_BITS)
    ...
    // Memory Storage
`ifdef MAX_MEM
    // lingkang: instantiate all memory (intended for Linux)
    reg     [BL_MAX*DQ_BITS-1:0] memory  [0:`MAX_SIZE-1];
`else
    // lingkang: only use part of the memory (intended for WindowsXP, windows can run large memory)
    // memory is "allocated" in task memory_write (see task memory_write/get_index())
    // However, if initializing memory using modelsim commands, memory_read won't return the initialized
    // data, and will work as if accessing an un-allocated location (see task memory_read/get_index())
...
`endif
    ...
    initial begin
        ...
        ////lingkang: use the golbal $timeformat
...
`ifdef DDR2_NO_TIMING_CHECK
        // lingkang: no_timing_check
`else
